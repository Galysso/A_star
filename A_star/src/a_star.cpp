			...
			if (ppDom != -1) {
				for (int m = 0; m < M; ++m) {
					...
					if (n == N) {
						...
						//double nouvLB = h->borneInfGLPK(node, ppDom, m);
						if ((nouvLB != -1) && (nouvLB < bestObj)) {
							...
							if (node->prof >= deepestProf) {
								...
								if ((UB != -1) && (UB < bestObj)) {
									...
									free(bestSol);
									bestSol = Usol;
									cout << endl << "(heuristique) nouvelle meilleure solution : (" << bestObj << ") ";
									for (int i = 0; i < N; ++i) {
										cout << (int) bestSol[i] << " ";
									}
									cout << endl;
								...
								if (UB != -1) {
									nouveau->selection = nouvLB + 1/(UB+0.5) - 1;
								}
								deepestProf = node->prof;
							}
							tas->empiler(nouveau);
						}
					}
